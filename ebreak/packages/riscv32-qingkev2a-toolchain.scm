(define-module (ebreak packages riscv32-qingkev2a-toolchain)
...
(define %target "riscv32-qingkev2a-elf")
...
;;;
(define-public riscv32-qingkev2a-picolibc
  (package
    (name "riscv32-qingkev2a-picolibc")
    (version "1.8.6")
    (source
     (origin
       (method git-fetch)
    ...
    (arguments
     (list
      #:build-type "release"
      #:configure-flags
      #~(list "--cross-file=../source/cross-riscv32-qingkev2a-elf.txt"
              "-Dmultilib=false"
          ...
          (add-before 'configure 'prepare-cross-file
            (lambda* (#:key inputs #:allow-other-keys)
              (let* ((xgcc (assoc-ref inputs "xgcc"))
                     ...
                     (cross-file "cross-riscv32-qingkev2a-elf.txt"))
                  ...
                  (call-with-output-file cross-file
                    (lambda (port)
                      (format port "[binaries]
                              ...
                              [properties]
                              c_args = ['-g', '-march=rv32ec_zicsr', '-mabi=ilp32e', '-msave-restore', '-fshort-enums']
                              c_link_args = ['-g', '-march=rv32ec_zicsr', '-mabi=ilp32e', '-msave-restore', '-fshort-enums']
...
package builds it for the riscv32-qingkev2a-elf target so it can be used as the
...
;;;
(define (gcc-cross-riscv32-qingkev2a-arguments base target)
...
(define-public gcc-cross-riscv32-qingkev2a
  (let ((base (cross-gcc %target
    ...
    (package
      (inherit base)
      (arguments (gcc-cross-riscv32-qingkev2a-arguments base %target))
      (native-inputs
       (modify-inputs (package-native-inputs base)
         (delete "xkernel-headers")
         (delete "libc:static"))))))

(define-public riscv32-qingkev2a-elf-toolchain
  (package
    (name "riscv32-qingkev2a-elf-toolchain")
    (version (package-version gcc-cross-riscv32-qingkev2a))
    (source #f)
          ...
          (let ((out #$output))
            ...
            (for-each
             ...
             (list #$(cross-binutils %target)
                   #$gcc-cross-riscv32-qingkev2a
                   #$riscv32-qingkev2a-picolibc))
            #t))))
    (propagated-inputs
     `(("binutils" ,(cross-binutils %target))
       ("gcc" ,gcc-cross-riscv32-qingkev2a)
       ("libc" ,riscv32-qingkev2a-picolibc)))
    (home-page "https://www.gnu.org/software/guix/")
    (synopsis "Complete riscv32-qingkev2a-elf cross toolchain")
    (description
