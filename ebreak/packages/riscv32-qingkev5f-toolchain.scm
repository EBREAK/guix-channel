(define-module (ebreak packages riscv32-qingkev5f-toolchain)
...
(define %target "riscv32-qingkev5f-elf")
...
;;;
(define-public riscv32-qingkev5f-picolibc
  (package
    (name "riscv32-qingkev5f-picolibc")
    (version "1.8.6")
    (source
     (origin
       (method git-fetch)
    ...
    (arguments
     (list
      #:build-type "release"
      #:configure-flags
      #~(list "--cross-file=../source/cross-riscv32-qingkev5f-elf.txt"
              "-Dmultilib=false"
          ...
          (add-before 'configure 'prepare-cross-file
            (lambda* (#:key inputs #:allow-other-keys)
              (let* ((xgcc (assoc-ref inputs "xgcc"))
                     ...
                     (cross-file "cross-riscv32-qingkev5f-elf.txt"))
...
package builds it for the riscv32-qingkev5f-elf target so it can be used as the
...
;;;
(define (gcc-cross-riscv32-qingkev5f-arguments base target)
...
(define-public gcc-cross-riscv32-qingkev5f
  (let ((base (cross-gcc %target
                         #:xgcc gcc-16
                         #:xbinutils (cross-binutils %target)
                         #:libc riscv32-qingkev5f-picolibc)))
    (package
      (inherit base)
      (arguments (gcc-cross-riscv32-qingkev5f-arguments base %target))
      (native-inputs
       (modify-inputs (package-native-inputs base)
         (delete "xkernel-headers")
         (delete "libc:static"))))))

(define-public riscv32-qingkev5f-elf-toolchain
  (package
    (name "riscv32-qingkev5f-elf-toolchain")
    (version (package-version gcc-cross-riscv32-qingkev5f))
    (source #f)
          ...
          (let ((out #$output))
            ...
            (for-each
             ...
             (list #$(cross-binutils %target)
                   #$gcc-cross-riscv32-qingkev5f
                   #$riscv32-qingkev5f-picolibc))
            #t))))
    (propagated-inputs
     `(("binutils" ,(cross-binutils %target))
       ("gcc" ,gcc-cross-riscv32-qingkev5f)
       ("libc" ,riscv32-qingkev5f-picolibc)))
    (home-page "https://www.gnu.org/software/guix/")
    (synopsis "Complete riscv32-qingkev5f-elf cross toolchain")
    (description
